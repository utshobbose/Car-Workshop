...
//       console.log('🔎 Loaded from localStorage:', { uid, r });
...
export default function AppointmentPage() {
      ...
      console.log('🔎 Loaded from localStorage:', { uid, r });
        ...
        .then(data => {
          if (data?.cars?.length > 0) {
            setSavedCar(data.cars[0]);
          } else {
            setSavedCar(null);
            setError('No car data found for this user');
          }
        })
        .catch(() => setError('Failed to fetch user car data'));
    }
  }, []);

  // 🔁 Load mechanics when date changes
  useEffect(() => {
    if (selectedDate) {
      const formattedDate = new Date(selectedDate).toISOString().split('T')[0];
      fetch(`${backend}/api/mechanics?date=${formattedDate}`)
  ...
  const handleSubmit = async (e) => {
    ...
    const finalCar = addNewCar ? newCarDetails : savedCar;
    ...
    if (addNewCar) {
      ...
      if (!savedCar) {
        setError('No car data available for appointment');
        setIsSubmitting(false);
        return;
      }
    }
